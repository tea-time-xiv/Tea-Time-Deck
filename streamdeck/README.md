...
| **Browser: Switch Type** | Cycles through the available types. |
...
**The Switch Type key is the one thing here worth configuring**, and only if you
want to: its inspector lists the types and ticking a few narrows the cycle to
those. Leave them all ticked — the default — and it cycles everything, including
types added by later versions. Useful once there are more types than you use, so
...
| `xiv.teatime.deck.sdPlugin/ui/` | Property inspectors, one per configurable action. |
